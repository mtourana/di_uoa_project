...
def compute_loss(model, x_gen, x_reco):
    ...
    #jets_pt = jet_pT(x_target).unsqueeze(1).cuda()
    #jets_mass = jet_mass(x_target).unsqueeze(1).cuda()

    x_gen_output = torch.zeros(batch_size,3,num_particles).cuda()
    x_gen_output = x_gen_decoded[:,0,:,:]
    #jets_pt_reco = jet_pT(x_gen_output).unsqueeze(1).cuda() 
    #jets_mass_reco = jet_mass(x_gen_output).unsqueeze(1).cuda()

    # MSE
    loss_rec_p = nn.functional.mse_loss(x_target, x_gen_output, reduction='sum')/batch_size
    # MSE
    #loss_rec_p = nn.functional.mse_loss(x_target, x_gen_output, reduction='sum')/batch_size

    # MAE Relative
    #loss_rec_p = nn.functional.l1_loss(x_target, x_gen_output, reduction='sum')/batch_size
