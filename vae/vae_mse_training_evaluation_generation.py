...
# Model params
model_name = '_sparse_mse_beta008_train_evaluate'+ str(num_particles) + 'p_jetpt_jetmass_'
num_classes = 1
latent_dim = 20
beta = 0.08
...
def compute_loss(model, x):
    mu_latent, logvar_latent = model.encode(x)
    z = model.reparameterize(mu_latent, logvar_latent )
    x_decoded = model.decode(z)
    x_pos = torch.zeros(batch_size,3,num_particles).cuda()
    x_pos = x[:,0,:,:]
    jets_pt = jet_pT(x_pos).unsqueeze(1).cuda()
    ...
    jets_pt_reco = jet_pT(x_decoded_pos).unsqueeze(1).cuda() 
    jets_mass_reco = jet_mass(x_decoded_pos).unsqueeze(1).cuda()

    # MSE
    loss_rec_p = nn.functional.mse_loss(x_pos, x_decoded_pos, reduction='sum')/batch_size

    # Function that takes the element-wise mean absolute error.
    #rel_mae_jet_pt = nn.functional.l1_loss(jets_pt, jets_pt_reco, reduction='mean')/jets_pt
    #rel_mae_jet_mass = nn.functional.l1_loss(jets_mass, jets_mass_reco, reduction='mean')/jets_mass

    # MSE jet features 
    mse_jet_pt = nn.functional.mse_loss(jets_pt, jets_pt_reco, reduction='mean')
    mse_jet_mass = nn.functional.mse_loss(jets_mass, jets_mass_reco, reduction='mean')
    
    loss_rec_j = gamma*(gamma_1*mse_jet_pt + gamma_2*mse_jet_mass)
    loss_rec = alpha*loss_rec_p + loss_rec_j

    # KLD
    loss_KLD = beta*(0.5 * torch.sum(torch.pow(mu_latent, 2) + torch.exp(logvar_latent) - logvar_latent - 1.0) / batch_size) 
    total_loss = loss_rec + loss_KLD

    return total_loss, loss_KLD, loss_rec, loss_rec_p, loss_rec_j, mse_jet_pt, mse_jet_mass
...
torch.save(px_train, 'px_beta008_latent20_std'+ str(model_name) + '.pt')
torch.save(py_train, 'py_beta008_latent20_std'+ str(model_name) + '.pt')
torch.save(pz_train, 'pz_beta008_latent20_std'+ str(model_name) + '.pt')

torch.save(px_reco_train, 'px_reco_beta008_latent20_std'+ str(model_name) + '.pt')
torch.save(py_reco_train, 'py_reco_beta008_latent20_std'+ str(model_name) + '.pt')
torch.save(pz_reco_train, 'pz_reco_beta008_latent20_std'+ str(model_name) + '.pt')

print('input shape: ', all_input.shape)
print('output shape', all_output.shape)

torch.save(all_input, 'all_input_beta008_latent20_std'+ str(model_name) + '.pt')
torch.save(all_output, 'all_output_beta008_latent20_std'+ str(model_name) + '.pt')
...
plt.legend()
plt.savefig('pxpypz_standardized_beta008_latent20' + str(model_name) + '.png')
plt.clf()
...
plt.xlabel('Epoch')
#plt.ylabel('A. U.')
plt.title('Dependent Components - MSE')
plt.legend()
plt.savefig('pxpypz_standardized_loss_components_latent20' + str(model_name) + '.png')
plt.clf()    

# Save the model
torch.save(model.state_dict(), 'model_pxpypz_standardized_MSELoss_beta008_latent20'+ str(model_name) + '.pt')
...
_ = plt.hist(mass_jet_reco_0, bins=100, range = [0, 400], histtype = 'step', density=False, label='Output Test VAE-MSE + Penalty (pt,mass)', color = 'black',linewidth=1.5)
...
_, bins, _ = plt.hist(mass_jet, bins=100, range = [0, 400], histtype = 'step', density=False, label='Input Test', color = spdred,linewidth=1.5)
_ = plt.hist(mass_jet_gen_0, bins=100, range = [0, 400], histtype = 'step', density=False, label='Randomly Generated VAE-MSE + Penalty (pt,mass)', color = 'black',linewidth=1.5)
plt.ylabel("Probability (a.u.)")
plt.xlabel('jet mass (GeV)')
plt.yscale('linear')
plt.legend(loc='lower right', prop={'size': 16})
...
_ = plt.hist(pt_jet_reco_0, bins=100, range=[0, 3000], histtype = 'step', density=False, label='Output Test VAE-MSE + Penalty (pt,mass)', color = 'black', linewidth=1.5)
plt.ylabel("Probability (a.u.)")
...
_, bins, _ = plt.hist(pt_jet, bins=100, range=[0, 3000], histtype = 'step', density=False, label='Input Test', color = spdred, linewidth=1.5)
_ = plt.hist(pt_jet_gen_0, bins=100, range=[0, 3000], histtype = 'step', density=False, label='Randomly Generated VAE-MSE + Penalty (pt,mass)', color = 'black', linewidth=1.5)
plt.ylabel("Probability (a.u.)")
plt.xlabel('jet $p_T$ (GeV)')
plt.yscale('linear')
#plt.xlim(-50.00, 50.00)
...
_ = plt.hist(e_jet_reco_0, bins=100, range = [200,4000], histtype = 'step', density=False, label='Output Test VAE-MSE + Penalty (pt,mass)', color = 'black', linewidth=1.5)
plt.ylabel("Probability (a.u.)")
...
_, bins, _ = plt.hist(e_jet, bins=100, range = [200,4000], histtype = 'step', density=False, label='Input Test', color = spdred, linewidth=1.5)
_ = plt.hist(e_jet_gen_0, bins=100, range = [200,4000], histtype = 'step', density=False, label='Randomly Generated VAE-MSE + Penalty (pt,mass)', color = 'black', linewidth=1.5)
plt.ylabel("Probability (a.u.)")
plt.xlabel('$jet energy$ (GeV)')
plt.yscale('linear')
#plt.xlim(-50.00, 50.00)
...
_ = plt.hist(eta_jet_reco_0, bins=80, range = [-3,3], histtype = 'step', density=False, label='Output Test VAE-MSE + Penalty (pt,mass)', color = 'black', linewidth=1.5)
plt.ylabel("Probability (a.u.)")
plt.xlabel('jet $\eta$')
plt.yscale('linear')
#plt.xlim(-50.00, 50.00)
...
_, bins, _ = plt.hist(eta_jet, bins=80, range = [-3,3], histtype = 'step', density=False, label='Input Test', color = spdred, linewidth=1.5)
_ = plt.hist(eta_jet_gen_0, bins=80, range = [-3,3], histtype = 'step', density=False, label='Randomly Generated VAE-MSE + Penalty (pt,mass)', color = 'black', linewidth=1.5)
plt.ylabel("Probability (a.u.)")
plt.xlabel('jet $\eta$')
plt.yscale('linear')
#plt.xlim(-50.00, 50.00)
...
_ = plt.hist(phi_jet_reco_0, bins=80, range=[-3,3], histtype = 'step', density=False, label='Output Test VAE-MSE + Penalty (pt,mass)', color = 'black', linewidth=1.5)
plt.ylabel("Probability (a.u.)")
plt.xlabel('jet $\phi$')
plt.yscale('linear')
#plt.xlim(-50.00, 50.00)
...
_, bins, _ = plt.hist(phi_jet, bins=80, range=[-3,3], histtype = 'step', density=False, label='Input Test', color = spdred, linewidth=1.5)
_ = plt.hist(phi_jet_gen_0, bins=80, range=[-3,3], histtype = 'step', density=False, label='Randomly Generated VAE-MSE + Penalty (pt,mass)', color = 'black', linewidth=1.5)
plt.ylabel("Probability (a.u.)")
plt.xlabel('jet $\phi$')
plt.yscale('linear')
#plt.xlim(-50.00, 50.00)
